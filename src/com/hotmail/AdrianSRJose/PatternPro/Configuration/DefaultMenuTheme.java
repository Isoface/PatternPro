...
public class DefaultMenuTheme extends MenuTheme
{
	private final static List<ItemConfig> items = new ArrayList<ItemConfig>();
	
	//
	public DefaultMenuTheme() 
	{
		super(items, Size.SIX_LINE.getSize(), "DefaultPatternTheme", Util.wc("&4&lDefaultPatternTheme"));
	}
	//
	@Override
	public List<ItemConfig> getItems()
	{
		...
		for (int x = 0; x < its.length ; x++)
			if (Arrays.asList(slots).contains(Integer.valueOf(x)))
				its[x] = new ItemConfig(x, "Item "+x+" Name Here", new ItemStack(Material.STAINED_GLASS_PANE, 1), (setBlack(x) ? (short)15 : (short)7), Arrays.asList(new String[]{" ", "This is the Item "+x+" Lore"}));
		//
		for (int x = 0; x < its.length; x++)
		{
			ItemConfig ii = its[x];
